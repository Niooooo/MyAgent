...
文件类工具统一限制在启动 `myagent` 时的工作目录内。工具定义和执行由 `ToolRegistry` 管理；新增工具时只需提供 schema 与 handler，不需要继续扩展 Agent Loop 的条件分支。
...
`bash` 工具仍只实现了临时的硬编码防护：它会拒绝同时带有递归和强制删除参数的 `rm` 命令，例如 `rm -rf`、`rm -fr`、`rm -r -f` 和 `rm --recursive --force`。

这不是完整的命令沙箱。其他有副作用的命令仍可能执行，因此不要把当前版本直接暴露给不受信任的用户。后续应把审批和安全策略迁移到独立 Hook。
