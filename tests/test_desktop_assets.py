...
class DesktopAssetTests(unittest.TestCase):
    ...
    def test_renderer_is_isolated_and_launcher_defaults_to_myagent_electron(self) -> None:
        ...
        self.assertIn("scripts\\bootstrap_desktop.py", launcher)
        self.assertIn('--project-root "%PROJECT_ROOT%."', launcher)
        self.assertIn('set "MYAGENT_PYTHON=', launcher)
        self.assertIn('if /i "%~1"=="--tk" goto tk_start', launcher)
        self.assertIn("MYAGENT_RUNTIME_HOME", self.main)
        self.assertIn("sidecarStderr", self.main)
        self.assertIn("this.failure || new Error", self.main)
